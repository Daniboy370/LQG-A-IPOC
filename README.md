...
### Introduction
The increasing adoption of autonomous systems across diverse domains has underscored the need for drift-resilient state estimation. Inertial Navigation Systems (INS) offer continuous position and velocity estimates, but their accuracy degrades over time due to sensor biases and noise. Although GPS is widely used for correction, its limited availability in many environments calls for alternative update sources—commonly referred to as "anchors of certainty". Among these, the Zero-Velocity Update (ZUPT) stands out for its effectiveness, providing accurate corrections during stationary intervals. However, conventional ZUPTs rely on nonholonomic constraints, restricting their use to surface-bound platforms. This work introduces a novel controlled extension, C-ZUPT, tailored for aerial platforms without surface dependence. By establishing an acceptable uncertainty tolerance, C-ZUPT identifies quasi-static equilibria to deliver precise velocity updates to the estimation filter. Extensive validation confirms that these opportunistic, high-quality updates substantially reduce INS drift and control effort. Consequently, C-ZUPT limits filter divergence and improves platform stability, leading to lower energy consumption and longer hover durations—key advantages for resource-constrained aerial systems.
